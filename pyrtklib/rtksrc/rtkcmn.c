...
*           2014/08/26 1.28 fix problem on output of rtkuncompress() for tar file
...
/* rtkuncompress file -------------------------------------------------------------
* rtkuncompress (rtkuncompress/unzip/uncompact hatanaka-compression/tar) file
* args   : char   *file     I   input file
*          char   *uncfile  O   rtkuncompressed file
* return : status (-1:error,0:not compressed file,1:rtkuncompress completed)
* note   : creates rtkuncompressed file in tempolary directory
*          gzip and crx2rnx commands have to be installed in commands path
*-----------------------------------------------------------------------------*/
extern int rtkuncompress(const char *file, char *uncfile)
{
    int stat=0;
    char *p,cmd[2048]="",tmpfile[1024]="",buff[1024],*fname,*dir="";
    
    trace(3,"rtkuncompress: file=%s\n",file);
    
    strcpy(tmpfile,file);
    if (!(p=strrchr(tmpfile,'.'))) return 0;
    
    /* rtkuncompress by gzip */
