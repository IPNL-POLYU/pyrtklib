...
0. Dependencies
    ...
    We don't provide pre-compiled package so far, thus, this library will be compiled in time. (Looking forward one day it can be directly installed by pip!) It requires several dependencies, including c compiler (gcc, clang are available, while msvc is not tested.) and cmake.
2. Clone from github
```shell
